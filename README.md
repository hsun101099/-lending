...
- Lucide Icons
- Recharts（主管報表圖表）
- Firebase（Firestore 資料庫 + Authentication 帳號密碼登入）
...
接著在「Users」頁籤手動建立同仁帳號（本系統不開放自行註冊）。

**3. 建立資料庫並套用安全規則**
「建構 → Firestore Database」建立資料庫，然後到「規則」頁籤，
把專案根目錄 [`firestore.rules`](./firestore.rules) 的內容整份貼上並發布。
...
| 集合 | 說明 |
|---|---|
| `cases/{caseId}` | 一筆案件，文件 ID 即案件編號（如 `LN-2026-1001`） |
...
- **登入**：帳號密碼登入，帳號由管理者在 Firebase Console 建立，不開放自行註冊。
